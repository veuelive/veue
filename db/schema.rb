...
ActiveRecord::Schema.define(version: 20_210_211_155_058) do
  ...
  create_table "follows", id: :uuid, default: -> { "gen_random_uuid()" }, force: :cascade do |t|
    ...
    t.index(
      %w[user_id channel_id unfollowed_at],
      name: "index_follows_on_user_id_and_channel_id_and_unfollowed_at",
      unique: true,
    )
  end
  ...
  create_table "video_events", id: :uuid, default: -> { "gen_random_uuid()" }, force: :cascade do |t|
    ...
    t.index(%w[video_id published timecode_ms type], name: "video_event_big_index")
    t.index(%w[video_id timecode_ms], name: "index_video_events_on_video_id_and_timecode_ms")
    t.index(%w[video_id type], name: "index_video_events_on_video_id_and_type")
  ...
  create_table "video_views", id: :uuid, default: -> { "gen_random_uuid()" }, force: :cascade do |t|
    ...
    t.index(%w[fingerprint video_id], name: "index_video_views_on_fingerprint_and_video_id")
    t.index(["user_joined_event_id"], name: "index_video_views_on_user_joined_event_id")
    t.index(
      %w[video_id details user_id],
      name: "index_video_views_on_video_id_and_details_and_user_id",
      unique: true,
    )
    t.index(%w[video_id last_seen_at], name: "index_video_views_on_video_id_and_last_seen_at")
    t.index(
      %w[video_id user_id],
      name: "index_video_views_on_video_id_and_user_id",
      unique: true,
      where: "(user_id IS NOT NULL)",
    )
    t.index(%w[video_id video_view_minutes_count], name: "index_video_views_on_video_id_and_video_view_minutes_count")
  end
  ...
  create_table "videos", id: :uuid, default: -> { "gen_random_uuid()" }, force: :cascade do |t|
    ...
    t.index(%w[state scheduled_at], name: "index_videos_on_state_and_scheduled_at")
