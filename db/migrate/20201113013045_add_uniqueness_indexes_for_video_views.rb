class AddUniquenessIndexesForVideoViews < ActiveRecord::Migration[6.0]
  def change
    remove_duplicates
    add_index :video_views, :user_id, unique: true, where: "user_id IS NOT NULL"
    add_index :video_views, :details, unique: true, where: "user_id IS NULL"
  end

  def remove_duplicates
    remove_duplicate_details_on_nil_users
    remove_duplicate_users
  end

  # Remove duplicate VideoView.details for nil users
  def remove_duplicate_details_on_nil_users
