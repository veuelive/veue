...
export default class ElectronCaptureSource extends VideoCaptureSource {
  ...
  static async connect(
    ...
    const deviceId = await ElectronCaptureSource.getWindowSource(windowTitle);
    console.log(
      `DesktopCapture selected deviceId ${deviceId} because it matched window title ${windowTitle}`
    );
    try {
      const source = new ElectronCaptureSource(deviceId);
