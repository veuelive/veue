...
describe "Scrubber previews" do
  ...
  describe "With snapshots" do
    ...
    it "Should show a video preview on mouse hover" do
      ...
      expect(page).to have_css(".progress-bar__video-preview__image[src*='/#{video.channel.slug}/videos/#{video.id}/snapshots/0.jpg'", wait: 5)
    end
    ...
    it "Should show the next snapshot at ~ 30 seconds" do
      ...
      expect(page).to have_css(".progress-bar__video-preview__image[src*='/#{video.channel.slug}/videos/#{video.id}/snapshots/1.jpg'")
    end
  end
